...
Since full node `v4.0.2`, we have introduced a `min-task-broadcast-interval` config to allow mining pools to customize the broadcast interval of mining tasks. You can configure this by adding `alephium.mining.min-task-broadcast-interval = xxx milli` to the `user.conf` file.
