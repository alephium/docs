...
There are also some arithmetic operators specifically for the `U256` type, here are some examples:

```rust
assert!(u256Max!() |+| 1 == 0, 0)
assert!(0 |-| 1 == u256Max!(), 0)
assert!(u256Max!() |*| 2 == u256Max!() - 1, 0)
assert!((1 << 128) |**| 2 == 0, 0)
```
...
The VM also provides two modulo instructions, `addModN` and `mulModN`:
