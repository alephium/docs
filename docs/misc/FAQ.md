...
```
3600 seconds / 16 seconds (Alephium block time) == 225 blocks per hour, per shard.
225 x 16 shards == 3600 blocks total per hour.
3600 x 24 hours == 86400 blocks per day.
86400 x  ALPH reward per block ~= number of ALPH mined per day.
```
